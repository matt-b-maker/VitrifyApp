...
export class FiringScheduleBuilderPage {
  ...
  @ViewChild(FiringScheduleComponent) firingScheduleComponent!: FiringScheduleComponent;
  ...
  chartData: any = this.transformFiringSchedule(this.firingScheduleService.firingScheduleBuildInProgress);

  options!: EChartsOption;

  constructor(public firingScheduleService: FiringScheduleService, private firingDetailsService: FiringDetailsService, private animationService: AnimationService, private alertController: AlertController) {
  ...
  async addSegment() {
    if (this.firingScheduleService.firingScheduleBuildInProgress.segments.length >= 1) {
      ...
      if (this.firingScheduleService.firingScheduleBuildInProgress.segments[lastSegmentIndex].hold) {
        this.firingScheduleService.firingScheduleBuildInProgress.segments.push({lowTemp: this.firingScheduleService.firingScheduleBuildInProgress.segments[lastSegmentIndex].lowTemp,
          highTemp: this.firingScheduleService.firingScheduleBuildInProgress.segments[lastSegmentIndex].lowTemp + 100, duration: '01:00', hold: false});
      } else {
        //set up the new segment and check for max temp reached
        let newSegment = {lowTemp: this.firingScheduleService.firingScheduleBuildInProgress.segments[lastSegmentIndex].highTemp, highTemp: this.firingScheduleService.firingScheduleBuildInProgress.segments[lastSegmentIndex].highTemp + 100, duration: '01:00', hold: false};
        if (newSegment.lowTemp === this.firingScheduleService.firingScheduleBuildInProgress.maxTemp) {
          await this.alertController.create({
            header: 'Max Temperature Reached',
            message: 'You have reached the maximum temperature for this firing schedule.',
            buttons: ['OK']
          }).then(alert => alert.present());
          return;
        }
        else if (newSegment.highTemp > this.firingScheduleService.firingScheduleBuildInProgress.maxTemp) {
          newSegment.highTemp = this.firingScheduleService.firingScheduleBuildInProgress.maxTemp;
        }
        this.firingScheduleService.firingScheduleBuildInProgress.segments.push(newSegment);
      }
    ...
    this.firingScheduleService.firingScheduleBuildInProgress.segments.push({lowTemp: 70, highTemp: 200, duration: '01:00', hold: false});
    this.hourInputs.push(1);
    this.minuteInputs.push(0);
    this.durationInputs.push('01:00');
  ...
  adjustTempScale(event: any) {
    let previousTempScale = this.firingScheduleService.firingScheduleBuildInProgress.tempScale;
    this.firingScheduleService.firingScheduleBuildInProgress.tempScale = event.detail.value;
    if (previousTempScale === 'F' && this.firingScheduleService.firingScheduleBuildInProgress.tempScale === 'C') {
      this.firingScheduleService.firingScheduleBuildInProgress.maxTemp = Math.round((this.firingScheduleService.firingScheduleBuildInProgress.maxTemp - 32) * 5 / 9);
      this.firingScheduleService.firingScheduleBuildInProgress.segments.forEach(segment => {
        segment.lowTemp = segment.lowTemp === 0 ? 0 : Math.round((segment.lowTemp - 32) * 5 / 9);
        segment.highTemp = Math.round((segment.highTemp - 32) * 5 / 9);
      });
    } else if (previousTempScale === 'C' && this.firingScheduleService.firingScheduleBuildInProgress.tempScale === 'F') {
      this.firingScheduleService.firingScheduleBuildInProgress.maxTemp = Math.round(this.firingScheduleService.firingScheduleBuildInProgress.maxTemp * 9 / 5 + 32);
      this.firingScheduleService.firingScheduleBuildInProgress.segments.forEach(segment => {
        segment.lowTemp = segment.lowTemp === 0 ? 0 : Math.round(segment.lowTemp * 9 / 5 + 32);
  ...
  updateChildChart() {
    this.chartData = this.transformFiringSchedule(this.firingScheduleService.firingScheduleBuildInProgress);
    this.firingScheduleComponent.updateChart(this.chartData, this.firingScheduleService.firingScheduleBuildInProgress, this.getTotalTime());
  }
  ...
  transformFiringSchedule(schedule: FiringSchedule) {
    ...
    schedule.segments.forEach(segment => {
        let durationMinutes = parseInt(segment.duration.split(':')[0]) * 60 + parseInt(segment.duration.split(':')[1]);
  ...
  getTotalTime() {
    let totalMinutes = this.firingScheduleService.firingScheduleBuildInProgress.segments.reduce((acc, segment) => {
      return acc + parseInt(segment.duration.split(':')[0]) * 60 + parseInt(segment.duration.split(':')[1]);
    }, 0);
